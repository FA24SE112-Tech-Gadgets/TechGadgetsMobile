...
export default function AddProduct({ navigation }) {
  ...
  const [dateChooseImportedDate, setDateChooseImportedDate] = useState(
    new Date()
  );
  const [dateChooseExpiredDate, setDateChooseExpiredDate] = useState(
    new Date()
  );
  ...
  const validateForm = () => {
    ...
    if (product.price === "" || product.price == undefined) {
      ...
      validationsString.price =
        "Giá phải lớn hơn 0 và đúng định dạng. VD: 10000";
      validations.price = true;
    } else {
    ...
    if (product.quantity === "" || product.quantity == undefined) {
      ...
      validationsString.quantity =
        "Số lượng phải lớn hơn 0 và đúng định dạng. VD: 10";
      validations.quantity = true;
    } else {
    ...
    if (product.sales === "" || product.sales == undefined) {
      ...
      validationsString.sales =
        "Giảm giá phải là số nguyên dương. VD: 35 => 35%";
      validations.sales = true;
    } else {
    ...
    if (product.importedDate !== "" && product.expiredDate !== "") {
      if (compareDates(product.importedDate, product.expiredDate) == 1) {
        validationsString.errorFinal =
          "Hạn sử dụng không được nhỏ hơn ngày nhập kho";
      ...
      >
        ...
        >
          ...
          <View style={styles.inputContainer}>
            <View style={styles.inputWrapper}>
              {errors.name && (
                <Text style={[styles.textError]}>{errorsString.name}</Text>
              )}
            </View>
          </View>
          ...
          <View style={styles.inputContainer}>
            <View style={styles.inputWrapper}>
              <TextInput
                style={[
                  styles.input,
                  errors.brandName ? styles.inputError : null,
                ]}
                placeholder="Hãng"
          ...
          <View style={styles.inputContainer}>
            <View style={styles.inputWrapper}>
              {errors.brandName && (
                <Text style={[styles.textError]}>{errorsString.brandName}</Text>
              )}
            </View>
          </View>
          ...
          >
            <View style={styles.filterWrapper}>
              ...
              <View style={styles.iconFilterContainer}>
                <AntDesign
                  name={isShowedCategory ? "caretup" : "caretdown"}
                  size={15}
                  color={errors.category ? "red" : "grey"}
                />
              </View>
            </View>
            {isShowedCategory && (
                    ...
                    style={[
                      styles.item,
                      {
                        ...
                        borderBottomLeftRadius:
                          index == arrCategoriesData.length - 1 ? 20 : 0,
                        ...
                        backgroundColor:
                          product.category._id === category._id
                            ...
                            : "#FEECE2",
                        borderTopWidth:
                          index == 0 || product.category._id === category._id
                            ? 1
                            : 0,
                        borderBottomWidth:
                          index == arrCategoriesData.length - 1 ||
                            product.category._id === category._id
                            ? 1
                            : 0,
                        borderLeftWidth: 1,
          ...
          <View style={styles.inputContainer}>
            <View style={styles.inputWrapper}>
              {errors.category && (
                <Text style={[styles.textError]}>{errorsString.category}</Text>
              )}
            </View>
          </View>
          ...
          {/* imageURL */}
          <Pressable
            style={[styles.filterContainer, { zIndex: 1 }]}
            onPress={() => {
          ...
          >
            <View style={styles.filterWrapper}>
              ...
              <View style={styles.iconFilterContainer}>
                <AntDesign
                  name={isShowedImg ? "caretup" : "caretdown"}
                  size={15}
                  color={errors.imageURL ? "red" : "grey"}
                />
              </View>
            </View>
            {isShowedImg && (
                  ...
                  <Pressable
                    style={[
                      styles.item,
                      {
                        ...
                        borderTopRightRadius: index == 0 ? 20 : 0,
                        borderBottomLeftRadius:
                          index == arrImages.length - 1 ? 20 : 0,
                        borderBottomRightRadius:
                          index == arrImages.length - 1 ? 20 : 0,
                        backgroundColor:
                          product.imageURL === url ? "#FFBE98" : "#FEECE2",
                        borderTopWidth:
                          index == 0 || product.imageURL === url ? 1 : 0,
                        borderBottomWidth:
          ...
          <View style={styles.inputContainer}>
            <View style={styles.inputWrapper}>
              {errors.imageURL && (
                <Text style={[styles.textError]}>{errorsString.imageURL}</Text>
              )}
            </View>
          </View>
          ...
          <View style={styles.inputContainer}>
            <View style={styles.inputWrapper}>
              {errors.price && (
                <Text style={[styles.textError]}>{errorsString.price}</Text>
              )}
            </View>
          </View>
          ...
          <View style={styles.inputContainer}>
            <View style={styles.inputWrapper}>
              {errors.quantity && (
                <Text style={[styles.textError]}>{errorsString.quantity}</Text>
              )}
            </View>
          </View>
          ...
          <View style={styles.inputContainer}>
            <View style={styles.inputWrapper}>
              {errors.sales && (
                <Text style={[styles.textError]}>{errorsString.sales}</Text>
              )}
            </View>
          </View>
          ...
          {/* importedDate */}
          <DatePickerCustom
            isError={errors.importedDate}
            date={dateChooseImportedDate}
            dateShow={
              product.importedDate.length == 0
                ? "Ngày nhập kho"
                : product.importedDate
            }
            onChange={handleChooseImportedDate}
          />
          <View style={styles.inputContainer}>
            <View style={styles.inputWrapper}>
              {errors.importedDate && (
                <Text style={[styles.textError]}>
                  {errorsString.importedDate}
                </Text>
              )}
            </View>
          </View>
          ...
          <DatePickerCustom
            ...
            date={dateChooseExpiredDate}
            dateShow={
              product.expiredDate.length == 0
                ? "Hạn sử dụng"
                : product.expiredDate
            }
            onChange={handleChooseExpiredDate}
          />
          <View style={styles.inputContainer}>
            <View style={styles.inputWrapper}>
              {errors.expiredDate && (
                <Text style={[styles.textError]}>
                  {errorsString.expiredDate}
                </Text>
              )}
            </View>
          </View>
          ...
          <ConfirmModal
            textTitle={"Bạn có chắc chắn tạo sản phẩm?"}
