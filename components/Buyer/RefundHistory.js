...
const RefundHistory = () => {
  ...
  const fetchTransactions = useCallback(async (pageNumber) => {
    ...
    setLoading(true);
    try {
      const response = await api.get(`/wallet-trackings?Types=Refund&Page=${pageNumber}&PageSize=10`);
  ...
  const renderItem = ({ item }) => (
    <View style={styles.transactionItem}>
      ...
      <Text style={styles.date}>Ngày hoàn tiền: {getRefundedAtText(item.refundedAt)}</Text>
  ...
  return (
    ...
    >
      <Text style={styles.title}>Lịch sử hoàn tiền</Text>
      <FlatList
        data={transactions}
