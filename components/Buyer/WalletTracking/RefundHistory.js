...
const RefundHistory = () => {
  ...
  const fetchTransactions = useCallback(async (pageNumber, sort = sortOption) => {
    ...
    setLoading(true);
    try {
      const response = await api.get(`/wallet-trackings?Types=Refund&SortByDate=${sort}&Page=${pageNumber}&PageSize=10`);
  ...
  const renderItem = ({ item, index }) => (
    <TouchableOpacity onPress={() => {
      if (item.sellerOrderId) {
        navigation.navigate('BuyerOrderDetail', { sellerOrderId: item.sellerOrderId });
      }
    }}>
      <View style={styles.transactionItem}>
        ...
        <Text style={styles.paymentMethod}>
          Loại giao dịch: <Text style={styles.paymentMethodValue}>Giao dịch tự động</Text>
        </Text>
        ...
        <View style={styles.amountDateContainer}>
          ...
          <View style={styles.dateContainer}>
            <Text style={styles.date}>Ngày hoàn: {getRefundedAtText(item.refundedAt)}</Text>
  ...
  const getRefundedAtText = (refundedAt) => {
    return refundedAt ? formatDate(refundedAt) : 'Đang chờ';
  };
    ...
    <View style={styles.container}>
      <LinearGradient colors={['#FFFFFF', '#fea92866']} style={styles.gradient}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Lịch sử hoàn tiền</Text>
        </View>
